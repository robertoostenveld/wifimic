...
const unsigned int nPage = 4;
const unsigned int nPacket = 400;      // the number of samples in a network packet can be up to 720 and still fit within the MTU of 1500 bytes
const unsigned int nBuffer = 1024;     // the number of samples in the I2S buffer is minimum 8, maximum 1024
...
struct {
  ...
  int16_t data[nPacket];
#endif
} packet __attribute__((packed));

int32_t *page[nPage];
int32_t samples[nPage];
uint8_t filling = 0;
uint8_t sending = 0;
...
void loop() {
  ...
  if ((millis() - lastBlink) > blinkInterval / 2) {
  ...
  if (samples[sending] == nPacket) {
...
#if defined(RTP_ULAW)
      packet.data[sample] = linear2ulaw(value); // this casts the value to an int16, and then converts to uint8
#elif defined(RTP_ALAW)
      packet.data[sample] = linear2alaw(value); // this casts the value to an int16, and then converts to uint8
#elif defined(RTP_L16)
      packet.data[sample] = value; // this casts the value to an int16
#endif
...
#ifdef PRINT_HEADER
    Serial.print(packet.version); Serial.print(", ");
    Serial.print(packet.type); Serial.print(", ");
    ...
    if (wifiConnected) {
      ...
      if (aboveThreshold) {
        ...
        Udp.endPacket();
        udpConnected = (count == sizeof(packet));
        if (!udpConnected) {
...
#endif
#ifdef USE_TCP
        count = Tcp.write((uint8_t *)(&packet), sizeof(packet));
        tcpConnected = (count == sizeof(packet));
        if (!tcpConnected) {
    ...
    // flag this page as empty
    samples[sending] = 0;

    // move on to the next page
    sending = (sending + 1) % nPage;
  }
